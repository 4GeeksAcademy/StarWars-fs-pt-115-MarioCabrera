    ...
    return (
        <div className="bg-white card text-center h-100 flex-shrink-0 bg-opacity-25" style={{ minWidth: "200px" }}>
            ...
            <div>
                ...
                {!favorite ? <button className="btn btn-warning mx-2 my-1" onClick={()=>{addFavorite(props.name)}}><i className="fa-regular fa-heart"></i></button>: <button className="btn btn-warning mx-2 my-1" onClick={()=>{removeFavorite(props.name)}}><i className="fa-solid fa-heart"></i></button>}
            </div>
        </div>
    );
};